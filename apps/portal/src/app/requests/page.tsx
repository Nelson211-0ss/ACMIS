...
export default async function RequestsPage() {
  ...
  const [institution, semester, registrations, specials, transfers, timeOff] = await Promise.all([
    client.public.institution().catch(() => null),
    client.reference.currentSemester().catch(() => null),
    client.students.myRegistrations().catch(() => []),
    client.lifecycle.specialExams({ mine_only: true, limit: 25 }).catch(() => null),
  ...
  return (
    <PortalShell user={user} institution={institution} currentPath="/requests">
      ...
      <section className="space-y-3">
        ...
        {requests.length === 0 ? (
          <p className="text-muted-foreground text-sm">You have not applied for any.</p>
        ) : (
          <ul className="space-y-3">
            {requests.map((request) => (
              <li key={request.id} className="bg-card shadow-card rounded-lg p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-sm font-medium">
                      <span className="font-mono text-xs">{request.reference}</span>
                      <span className="ml-2">{humaniseStatus(request.kind)} examination</span>
                    </p>
                    <p className="text-muted-foreground mt-0.5 text-xs">
                      On the ground of {humaniseStatus(request.ground).toLowerCase()}
                      {request.missed_on ? ` · missed ${date(request.missed_on)}` : ""} · lodged{" "}
                ...
                <dl className="text-muted-foreground mt-3 grid gap-3 border-t pt-3 text-xs sm:grid-cols-3">
                  ...
                  <div>
                    <dt>Fee</dt>
                    <dd className="text-foreground mt-0.5 font-medium">
                      {request.fee_minor ? money(request.fee_minor, currency) : "None charged"}
                    </dd>
                  </div>
                </dl>
                ...
                {request.decision_note ? (
                  <p className="text-muted-foreground bg-muted/40 mt-3 rounded-md border p-3 text-xs">
                    {request.decision_note}
                    {request.minute_reference ? ` (minute ${request.minute_reference})` : ""}
                  </p>
                ) : null}

                {!request.evidence_verified_at && request.evidence_attachment_ids.length === 0 ? (
                  <p className="text-warning-foreground mt-3 text-xs">
                    This cannot be granted until the evidence reaches the faculty office. Lodging
                    the application does not put the evidence in.
                  </p>
                ) : null}
              </li>
            ))}
      ...
      <section className="space-y-3">
        ...
        {!semester ? (
          <p className="text-muted-foreground text-sm">
            No semester is currently running, so there is nothing to apply against.
          </p>
        ) : courses.length === 0 ? (
          <EmptyState
          ...
          <div className="bg-card shadow-card rounded-lg p-4">
            <p className="text-muted-foreground mb-4 text-sm">
              Applications are decided by the faculty board on the facts and the evidence, in that
              order. Apply as soon as you can: a late application is refused on lateness alone even
              where the ground was good.
            </p>
        ...
        <section className="space-y-3">
          ...
          <div className="bg-card shadow-card rounded-lg p-4">
            ...
            <p className="text-muted-foreground mt-3 text-xs leading-relaxed">
              A dead semester is taken before the semester starts, at the registry, with a reason.
              Simply not registering is not a dead semester — it counts against your maximum period
              of study and can end in discontinuation. If you need to stop, say so.
            </p>
          </div>
        </section>
      ) : null}
        ...
        <section className="space-y-3">
          ...
          <ul className="space-y-3">
            {transfers.map((transfer) => (
              <li key={transfer.id} className="bg-card shadow-card rounded-lg p-4">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    <p className="text-sm font-medium">
                    ...
                    <p className="text-muted-foreground mt-0.5 text-xs">
                      {transfer.other_programme_name ?? "Programme not stated"} · requested{" "}
          ...
          <p className="text-muted-foreground text-xs leading-relaxed">
            A transfer is not complete until the receiving institution accepts the credit
            assessment. Credit is capped as a proportion of the programme — an institution cannot
            award its degree on a majority of someone else&rsquo;s teaching.
          </p>
        </section>
      ) : null}
    </PortalShell>
